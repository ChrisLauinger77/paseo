...
test("concurrent child completions each notify their shared parent", async () => {
  const childAgents = new Map<string, ManagedAgent>();
  for (const childAgentId of ["child-one", "child-two"]) {
    const childAgent: ManagedAgent = Object.create(null);
    Reflect.set(childAgent, "id", childAgentId);
    Reflect.set(childAgent, "lifecycle", "idle");
    Reflect.set(childAgent, "config", { title: childAgentId });
    Reflect.set(childAgent, "pendingPermissions", new Map());
    childAgents.set(childAgentId, childAgent);
  }

  const parentAgent: ManagedAgent = Object.create(null);
  Reflect.set(parentAgent, "id", "parent-agent");
  Reflect.set(parentAgent, "lifecycle", "running");
  Reflect.set(parentAgent, "config", { title: "Parent Agent" });

  const subscribers = new Map<string, (event: AgentManagerEvent) => void>();
  const parentPrompts: string[] = [];
  const agentManager = new AgentManager({ clients: {}, logger: createTestLogger() });
  Reflect.set(agentManager, "getAgent", (agentId: string) => {
    return agentId === "parent-agent" ? parentAgent : (childAgents.get(agentId) ?? null);
  });
  Reflect.set(
    agentManager,
    "subscribe",
    (callback: (event: AgentManagerEvent) => void, options: { agentId: string }) => {
      ...
      return () => subscribers.delete(options.agentId);
    },
  );
  ...
  Reflect.set(agentStorage, "get", async (agentId: string) => {
    if (agentId === "parent-agent") {
      return { title: "Parent Agent", labels: {} };
    }
  ...
  for (const childAgentId of childAgents.keys()) {
    setupFinishNotification({
      agentManager,
      agentStorage,
      childAgentId,
  ...
  for (const [childAgentId, childAgent] of childAgents) {
    childAgent.lifecycle = "running";
    subscribers.get(childAgentId)?.({ type: "agent_state", agent: childAgent });
  }
  for (const [childAgentId, childAgent] of childAgents) {
    childAgent.lifecycle = "idle";
    subscribers.get(childAgentId)?.({ type: "agent_state", agent: childAgent });
  ...
  expect(parentPrompts).toEqual(
    expect.arrayContaining([
      formatSystemNotificationPrompt("Agent child-one (child-one) finished."),
      formatSystemNotificationPrompt("Agent child-two (child-two) finished."),
    ]),
  );
});
